...
async function enumLabelExists(
    client: Client,
    enumName: string,
    label: string,
): Promise<boolean> {
...
export async function migrateRoleEnumBeforeSync(
    options: DataSourceOptions,
): Promise<void> {
    const client = new Client(getPgClientOptions(options));
    ...
    try {
        ...
        const legacyRoleIds = await client.query<{ id: number }>(`
            SELECT id
            FROM roles
        ...
        const employeeRole = await client.query<{ id: number }>(`
            SELECT id
            FROM roles
